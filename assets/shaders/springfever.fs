...
extern MY_HIGHP_OR_MEDIUMP vec2 springfever;        // Parámetro del shader (nombre debe coincidir con el archivo)
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    float speed = springfever.x * 0.05;     // velocidad (springfever.x entre 0 y 2 aproximadamente)
    float intensity = springfever.y * 0.1;       // intensidad de la niebla (0.0 - 1.0)
    ...
    vec3 colorA = vec3(0.45, 0.15, 0.60);  // Púrpura medio
    vec3 colorB = vec3(0.70, 0.30, 0.90);  // Púrpura claro
    vec3 colorC = vec3(0.20, 0.05, 0.30);  // Púrpura oscuro (en lugar del azul oscuro original)
    vec3 colorD = vec3(0.80, 0.60, 1.00);  // Lila claro (reemplazando cyan claro)
    ...
    float fogAlpha = clamp(intensity * brightness, 0.1, 0.7);
